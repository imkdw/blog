...
export const clearDatabase = async () => {
  const rawQuery = `SELECT table_name FROM information_schema.tables WHERE table_schema = 'imkdw_dev';`;
  const tables: { TABLE_NAME: string }[] = await prisma.$queryRaw`${rawQuery}`;
