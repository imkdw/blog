...
export default class ArticleViewTrendRepository implements IArticleViewTrendRepository {
  ...
  async findLastOne(option: FindOption): Promise<ArticleViewTrendEntity | null> {
    ...
    return row ? new ArticleViewTrendEntity(row) : null;
  }

  async create(data: ArticleViewTrendEntity): Promise<ArticleViewTrendEntity> {
    const row = await this.prisma.articleViewTrend.create({ data });
    return new ArticleViewTrendEntity(row);
  }
}
